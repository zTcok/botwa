...
let handler = async (m, { conn, text }) => {
	...
	if (!(text.startsWith("08") || text.startsWith("62"))) {
		return m.reply("Hanya bisa nomor Indonesia.");
	}

	text = text.startsWith("08") ? text.replace("08", "62") : text;
	if (text + "@s.whatsapp.net" === conn.user.jid) {
		return m.reply("Sepertinya Anda memasukkan nomor Bot.");
	}

	const isValid = await conn.onWhatsApp(text + "@s.whatsapp.net");
	if (isValid.length == 0) {
		return m.reply("Nomor yang anda masukan tidak terdaftar di Whatsapp, silahkan coba lagi.");
	}
